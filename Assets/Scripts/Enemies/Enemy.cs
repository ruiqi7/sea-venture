...
public class Enemy : MonoBehaviour
{
    ...
    [Tooltip("The enemy is initially facing left.")]
    ...
    private void MoveEnemy()
    {
        ...
        Vector3 scale = GetDesiredScale(movement.x);
    ...
    protected virtual Vector3 GetDesiredMovement()
    {
        ...
        switch(movementMode)
        {
            case MovementModes.FollowTarget:
                if ((followTarget.position - transform.position).magnitude < followRange)
                {
    ...
    /// <summary>
    /// Description:
    /// Calculates and returns the desired scale of this enemy
    /// Inputs: 
    /// none
    /// Returns: 
    /// Vector3
    /// </summary>
    /// <returns>Vector3: The desired scale</returns>
    protected virtual Vector3 GetDesiredScale(float moveDirection)
    {
        Vector3 scale;
        switch(movementMode)
        {
            case MovementModes.FollowTarget:
            case MovementModes.Scroll:
                if ((moveDirection > 0 && facingLeft) || (moveDirection < 0 && !facingLeft))
                {
                    scale = GetFlippedScale();
    ...
    /// <summary>
    /// Description:
    /// The desired scale of the enemy after flipping
    /// Inputs: 
    /// none
    /// Returns: 
    /// Vector3
    /// </summary>
    /// <returns>Vector3: The flipped scale.</returns>
